...
const filteredNews = computed(() => {
  ...
  if (filterStatus.value === 'inactive') list = list.filter(a => !a.isActive)
  if (filterStatus.value === 'trending') list = list.filter(a => a.isTrending)
  if (filterStatus.value === 'featured') list = list.filter(a => a.isFeatured)
  return list
})
...
async function loadNews() {
  ...
  try {
    ...
    news.value = data && data.length ? data : []
    if (!news.value.length && page.value === 0) news.value = mockArticles
  } catch { news.value = mockArticles }
  finally { loading.value = false }
}

function onSearch() { /* reactive computed handles it */ }

async function toggleTrending(article) {
  try { await newsAPI.toggleTrending(article.id); await loadNews() }
  catch { alert('API error') }
}

async function toggleActive(article) {
  try {
    if (article.isActive) await newsAPI.disable(article.id)
    else await newsAPI.enable(article.id)
    await loadNews()
  } catch { alert('API error') }
}
...
async function deleteArticle(id) {
  ...
  try { await newsAPI.remove(id); await loadNews() }
  catch { alert('API error') }
}
